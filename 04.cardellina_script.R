...
sp1 <- read.csv("SMOC/SMOC_joint.csv", h=T)
sp2 <- read.csv("SMS/SMS_joint.csv", h=T)
sp3 <- read.csv("TMVB/TMVB_joint.csv", h=T)
...
varclim1 <- list.files("E:/cardellina/SMOC/M_variables/Set_1/", pattern=".asc", full.names=T)
varclim2 <- list.files("E:/cardellina/SMS/M_variables/Set_1/", pattern=".asc", full.names=T)
varclim3 <- list.files("E:/cardellina/TMVB/M_variables/Set_1/", pattern=".asc", full.names=T)
...
ecospat.niche.overlap (z1=z1_grid, z2=z2_grid, cor=TRUE)  #SMOC and SMS
ecospat.niche.overlap (z1=z1_grid, z2=z3_grid, cor=TRUE) #SMOC and TMVB  
ecospat.niche.overlap (z1=z2_grid, z2=z3_grid, cor=TRUE) #SMS and TMVB  

ecospat.niche.overlap (z1=z2_grid, z2=z1_grid, cor=TRUE)  #SMS and SMOC
ecospat.niche.overlap (z1=z3_grid, z2=z1_grid, cor=TRUE) #TMVB and SMOC  
ecospat.niche.overlap (z1=z3_grid, z2=z2_grid, cor=TRUE) #TMVB and SMS  


### Occurrence density graphics in the environmental space.
windows() 
par(mfrow=c(1,3))
ecospat.plot.niche (z1_grid, title="SMOC", name.axis1="PC1", name.axis2="PC2", cor=F)
ecospat.plot.niche (z2_grid, title="SMS", name.axis1="PC1", name.axis2="PC2", cor=F)
ecospat.plot.niche (z3_grid, title="TMVB", name.axis1="PC1", name.axis2="PC2", cor=F)

### Individual niche plots. Modifications of plot.niche functions from Broenniman et al. 2012 and Silva et al. 2014.
n.groups <- 3
g.names <- c("SMOC", "SMS", "TMVB")
g.colors <- c("darkblue", "red", "brown")
